...
export async function interpretActivity(input: {
  ...
  const result = streamText({
    ...
    system: `Write directly to the roommate, warmly and briefly. When no sources are attached, summary MUST be an empty string (including multiple or unsupported activities): the server provides the useful next question or confirmation. Never narrate "The user states", list missing fields, or repeat their message. For a request referencing documents, write a short Markdown summary of relevant source facts (at most 40 words, one or two short sentences), citing [Source 1] etc. Do not invent links, embed images, expose reasoning, claim anything was recorded, or predict server validation. Include only facts that help answer the request; no headings, preamble, process commentary, or repeated confirmation reminders. The server supplies the next question and confirmation separately. Then extract the activity.
...
Set incomplete when payer, contribution amount, or bill reference is missing, or when meaning/temporal reference remains unclear. Missing total or dueDate ALONE does not mean incomplete: the server will first look for an existing bill and ask for creation details if needed. If share-versus-provider semantics are ambiguous, return unsupported rather than guessing. Do not invent names, totals, dates, or bill references.`,
  ...
  for await (const partial of result.partialOutputStream) {
    ...
    if (summary.startsWith(emitted) && summary.length <= 1800) {
      // Simple activities use the authoritative resolver response only.
      if (sources.length) input.onSummary?.(summary.slice(emitted.length));
      emitted = summary;
    }
  }
