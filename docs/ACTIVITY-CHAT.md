...
- `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, and `pnpm build` passed. The source suite is updated with streamed-provider, PDF/text extraction, source-limit and upload-boundary tests. Ordinary CI skips real database and live-model tests.
